...
class VigilanteService {
    ...
    async procesarCambioEstado(registroId, nuevoEstado, vigilanteId) {
        ...
        try {
            ...
            const registro = await vigilanteRepo.obtenerRegistroPorId(connection, registroId);
            if (!registro) {
                throw new Error('Registro de dispositivo no encontrado');
            }

            await vigilanteRepo.actualizarEstadoRegistro(connection, registroId, nuevoEstado);

            if (nuevoEstado === 2) {
                await vigilanteRepo.registrarEntradaAsistencia(connection, registro.alumno_id, vigilanteId);
            } else if (nuevoEstado === 3) {
                await vigilanteRepo.registrarSalidaAsistencia(connection, registro.alumno_id);
            }
