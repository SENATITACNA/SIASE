const datosAlumnosService = require("../services/datos_alumno.service");
const obtenerAlumnoPorId = (req, res) => {
    ...
    datosAlumnosService.obtenerAlumnoPorId(id, (err, alumno) => {
        ...
        else if (alumno.length === 0) res.status(404).json({ mensaje: "Alumno no encontrado" });
        else res.json(alumno[0]);
    });
};
