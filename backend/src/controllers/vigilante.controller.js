    ...
    async putEstadoRegistro(req, res) {
        try {
            ...
            if (!nuevo_estado || !vigilante_id) {
                return res.status(400).json({ mensaje: 'Faltan datos requeridos (nuevo_estado, vigilante_id)' });
            }

            const resultado = await vigilanteService.procesarCambioEstado(registroId, nuevo_estado, vigilante_id);
